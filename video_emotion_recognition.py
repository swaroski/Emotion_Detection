...
def gen():
    ...
    def detect_face(frame):
        ...
        cascPath = (".\\data\\face_landmarks.dat")
        ...
        print("SHEELA- detect_face returned")                                                  
    ...
    model = load_model(".\\data\\video.h5", compile = False)
    
    # Load the face detector
    face_detect = dlib.get_frontal_face_detector()
    
    # Load the facial landmarks predictor
    predictor_landmarks  = dlib.shape_predictor(".\\data\\face_landmarks.dat")
    ...
    while end - start < max_time :
        ...
        cv2.imwrite(".\\tmp\\t.jpg", frame)
        
        # Yield the image at each step
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + open(".\\tmp\\t.jpg", 'rb').read() + b'\r\n')
        ...
        if end-start > max_time - 1 :
            with open(".\\static\\db\\histo_perso.txt", "w") as d:
                d.write("density"+'\n')
                for val in predictions :
                    d.write(str(val)+'\n')
                d.close()
            with open(".\\static\\db\\histo.txt", "a") as d:
                for val in predictions :
                    d.write(str(val)+'\n')
                d.close()
            ...
            import csv
            with open(".\\static\\db\\prob.csv", "w") as d:
                writer = csv.writer(d)
                for row in rows:
                    writer.writerow(row)
                d.close()

            with open(".\\static\\db\\prob_tot.csv", "a") as d:
                writer = csv.writer(d)
                for row in rows:
                    writer.writerow(row)
                d.close()
